...
final class Class39
{
	...
	public void method545(Buffer buffer, Class29 class29)
	{
		...
		if(i != 0)
		{
			anIntArray668[0] = buffer.readUnsignedWord();
			anIntArray668[1] = buffer.readUnsignedWord();
			int j = buffer.readUnsignedByte();
			for(int k = 0; k < 2; k++)
			{
				for(int l = 0; l < anIntArray665[k]; l++)
				{
					anIntArrayArrayArray666[k][0][l] = buffer.readUnsignedWord();
					anIntArrayArrayArray667[k][0][l] = buffer.readUnsignedWord();
				}
			...
			for(int i1 = 0; i1 < 2; i1++)
			{
				for(int j1 = 0; j1 < anIntArray665[i1]; j1++)
					if((j & 1 << i1 * 4 << j1) != 0)
					{
						anIntArrayArrayArray666[i1][1][j1] = buffer.readUnsignedWord();
						anIntArrayArrayArray667[i1][1][j1] = buffer.readUnsignedWord();
					} else
					{
