...
public final class TextDrawingArea extends DrawingArea {
	  ...
	  public void drawRightAlignedString(String s, int x, int y, int color) {
		...
		for (int i1 = 0; i1 < s.length(); i1++) {
		    char c = s.charAt(i1);
		    if (c != ' ')
			method392(aByteArrayArray1491[c], x + anIntArray1494[c], y+ anIntArray1495[c], anIntArray1492[c],anIntArray1493[c], color);x += anIntArray1496[c];
		}
	    }
	...
	public void drawCenteredString(int x, int y, String text, int color, boolean shadow) {
        drawLeftAlignedString(x - getTextWidth(text) / 2, y, text, color, shadow);
    }
	
	public void drawLeftAlignedString(int x, int y, String text, int color, boolean shadow) {
		aBoolean1499 = false;
        final int l = x;
        if (text == null) {
            return;
        ...
        for (int i1 = 0; i1 < text.length(); i1++) {
            if (text.charAt(i1) == '@' && i1 + 4 < text.length() && text.charAt(i1 + 4) == '@') {
                final int j1 = getColorByName(text.substring(i1 + 1, i1 + 4));
                if (j1 != -1) {
                    color = j1;
                }
                i1 += 4;
            } else {
                final char c = text.charAt(i1);
                if (c != ' ') {
                    if (shadow) {
                    	method392(aByteArrayArray1491[c], x + anIntArray1494[c] + 1, y + anIntArray1495[c] + 1, anIntArray1492[c],
                        		anIntArray1493[c], 0);
                    }
                    method392(aByteArrayArray1491[c], x + anIntArray1494[c], y + anIntArray1495[c], anIntArray1492[c], anIntArray1493[c],
                            color);
                }
                x += anIntArray1492[c];
            }
        ...
        if (aBoolean1499) {
            DrawingArea.drawHorizontalLine(l, y + (int) (anInt1497 * 0.69999999999999996D), x - l, 0x800000);
        }
    }
	...
	private void method392(byte abyte0[], int i, int j, int k, int l, int i1) {
        int j1 = i + j * canvasWidth;
        int k1 = canvasWidth - k;
        int l1 = 0;
        int i2 = 0;
        if (j < clipStartY) {
            int j2 = clipStartY - j;
        ...
        if (!(k <= 0 || l <= 0)) {
            method393(canvasRaster, abyte0, i1, i2, j1, k, l, k1, l1);
        }
    }
	...
	private void method394(int i, int j, byte abyte0[], int k, int l, int i1,
                           int j1) {
        int k1 = j + l * canvasWidth;
        int l1 = canvasWidth - k;
        int i2 = 0;
        int j2 = 0;
        if (l < clipStartY) {
            int k2 = clipStartY - l;
        ...
        if (k <= 0 || i1 <= 0)
            return;
        method395(abyte0, i1, k1, canvasRaster, j2, k, i2, l1, j1, i);
    }
