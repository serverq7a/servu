final class Censor {
	...
	public static void loadConfig(Archive jagexArchive)
	{
		Buffer buffer = new Buffer(jagexArchive.getDataForName("fragmentsenc.txt"));
		Buffer stream_1 = new Buffer(jagexArchive.getDataForName("badenc.txt"));
		Buffer stream_2 = new Buffer(jagexArchive.getDataForName("domainenc.txt"));
		Buffer stream_3 = new Buffer(jagexArchive.getDataForName("tldlist.txt"));
		readValues(buffer, stream_1, stream_2, stream_3);
	}

	private static void readValues(Buffer buffer, Buffer stream_1, Buffer stream_2, Buffer stream_3)
	{
		readBadEnc(stream_1);
	...
	{
		int i = buffer.readDWord();
		aCharArrayArray624 = new char[i][];
		anIntArray625 = new int[i];
		for(int j = 0; j < i; j++)
		{
			anIntArray625[j] = buffer.readUnsignedByte();
	...
	{
		anIntArray620 = new int[buffer.readDWord()];
		for(int i = 0; i < anIntArray620.length; i++)
			anIntArray620[i] = buffer.readUnsignedWord();
	}
