package org.mistex.system.util.control;
...
public class PanelSettings {
	...
	public Client getClient(int id) {
		return (Client) Mistex.playerHandler.players[id];
	}
	public boolean validClient(int id) {
		if(id < 0 || id > MistexConfiguration.MAX_PLAYERS)
			return false;
	...
	public boolean validNpc(int index) {
		if (index < 0 || index > MistexConfiguration.MAX_NPCS) {
			return false;
		}
		NPC n = getNpc(index);
		if (n != null) {
	...
	public NPC getNpc(int index) {
		return (NPC) Mistex.npcHandler.npcs[index];
	}
	...
	public void executeCommand(String cmd) {
		...
		if (cmd.equalsIgnoreCase("Send")) {
			if (p.NPC_CHECKBOX.isSelected()) {
				for (int i = 0; i < MistexConfiguration.MAX_NPCS; i++) {
					if (validNpc(i)) {
						NPC n = getNpc(i);
						if(inList(""+n.npcType))
		...
		if (cmd.equalsIgnoreCase("Update")) {
			Mistex.playerHandler.updateSeconds = (Integer.parseInt(p.UPDATE_SERVER_TEXT.getText()));
			Mistex.playerHandler.updateAnnounced = false;
			Mistex.playerHandler.updateRunning = true;
			Mistex.playerHandler.updateStartTime = System.currentTimeMillis();
			return;
		}
		if (cmd.equalsIgnoreCase("Reset")) {
			...
			for (int i = 0; i < MistexConfiguration.MAX_NPCS; i++) {
				boolean skip = false;
				NPC n = Mistex.npcHandler.npcs[i];
				if (n != null) {
					for (int k : SKIP_NPCS) {
						if (n.npcType == k) {
							skip = true;
		...
		if (cmd.equalsIgnoreCase("Update Settings")) {
			boolean active = MistexConfiguration.DOUBLE_EXP;
			MistexConfiguration.SERVER_NAME = p.SERVER_NAME_TEXT.getText();
			MistexConfiguration.LOGOUT_MESSAGE = p.LOGOUT_BUTTON_TEXT.getText();
			MistexConfiguration.DEATH_MESSAGE = p.DEATH_MESSAGE_TEXT.getText();
			MistexConfiguration.ADMIN_CAN_TRADE = p.ADMINS_CAN_TRADE.isSelected();
			MistexConfiguration.ADMIN_DROP_ITEMS = p.ADMINS_CAN_DROP.isSelected();
			MistexConfiguration.ADMIN_CAN_SELL_ITEMS = p.ADMINS_CAN_SELL_ITEMS.isSelected();
			MistexConfiguration.MINI_GAMES = p.MINI_GAMES.isSelected();
			MistexConfiguration.LOCK_EXPERIENCE = p.LOCK_EXPERIENCE.isSelected();
			MistexConfiguration.DOUBLE_EXP = p.DOUBLE_EXPERIENCE.isSelected();

			for (int i = 0; i < MistexConfiguration.MAX_PLAYERS; i++) {
				if (validClient(i)) {
					Client c2 = getClient(i);
					c2.getPA().sendFrame126(MistexConfiguration.LOGOUT_MESSAGE, 2458);
				}
			}
			return;
		}
		...
		if (p.ENTITY_LIST.getSelectedValue() != null) {
			...
			if (cmd.equalsIgnoreCase("Ban Player")) {
				PunishmentHandler.addNameToBanList(c.playerName);
				PunishmentHandler.addNameToFile(c.playerName);
				c.disconnected = true;
				return;
			}
			if (cmd.equalsIgnoreCase("IP-Ban")) {
				PunishmentHandler.addIpToBanList(c.connectedFrom);
				PunishmentHandler.addIpToFile(c.connectedFrom);
				c.disconnected = true;
				return;
			}
			...
			if (cmd.equalsIgnoreCase("IP-Mute")) {
				PunishmentHandler.addIpToMuteList(c.connectedFrom);
				c.sendMessage("You have been muted.");
				return;
			}
			...
			if (cmd.equalsIgnoreCase("Drop Item")) {
				...
				if (item <= 160000 && item > 0) {
					if (c.getItems().playerHasItem(item)) {
						...
						Mistex.itemHandler.createGroundItem(c, item, c.getX(), c.getY(), c.playerItemsN[slot], c.getId());
	...
	public void playerCommand(String cmd, Client c) {
		...
		if (cmd.equals("Force Shop")) {
			/*int shop = -1;
			String name = (String)JOptionPane.showInputDialog(null, "Chose the Equipment you wish to remove:\n", "Magic Bomber", -1, null, Mistex.shopHandler.ShopName, Mistex.shopHandler.ShopName);
			for (int i = 0; i < Mistex.shopHandler.ShopName.length; i++) {
				if(name.equals(Mistex.shopHandler.ShopName[i]))
					shop = i;
			}
			if(shop != -1)
