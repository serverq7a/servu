...
public class AttackPlayer implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case ATTACK_PLAYER:
			...
			if ((usingBow || c.autocasting) && c.goodDistance(c.getX(), c.getY(),Mistex.playerHandler.players[c.playerIndex].getX(),Mistex.playerHandler.players[c.playerIndex].getY(),6)) {
				c.usingBow = true;
				c.stopMovement();
			}

			if (usingOtherRangeWeapons && c.goodDistance(c.getX(), c.getY(),Mistex.playerHandler.players[c.playerIndex].getX(),Mistex.playerHandler.players[c.playerIndex].getY(),3)) {
			...
			if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows]
					&& PlayerConfiguration.CORRECT_ARROWS
					&& usingBow
		...
		case MAGE_PLAYER:
			...
			for (int r = 0; r < c.REDUCE_SPELLS.length; r++) { // reducing
																// spells,
																// confuse etc
				if (Mistex.playerHandler.players[c.playerIndex].REDUCE_SPELLS[r] == c.MAGIC_SPELLS[c.spellId][0]) {
					if ((System.currentTimeMillis() - Mistex.playerHandler.players[c.playerIndex].reduceSpellDelay[r]) < Mistex.playerHandler.players[c.playerIndex].REDUCE_SPELL_TIME[r]) {
			...
			if (System.currentTimeMillis()
					- Mistex.playerHandler.players[c.playerIndex].teleBlockDelay < Mistex.playerHandler.players[c.playerIndex].teleBlockLength
			...
			if (c.usingMagic) {
				if (c.goodDistance(c.getX(), c.getY(),
						Mistex.playerHandler.players[c.playerIndex].getX(),
						Mistex.playerHandler.players[c.playerIndex].getY(), 7)) {
					c.stopMovement();
				}
				if (c.getCombat().checkReqs()) {
					c.followId = c.playerIndex;
