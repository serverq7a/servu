...
public class MeleeMaxHit {
	...
	public static double calculateBaseDamage(Client c, boolean special) {
		...
		if (hasKorasi(c) && c.usingSpecial) {
			strength += (int)(lvlForXP * .80 * 2);
		}

		if (c.slayerHelmAffect) {
			base = (base * 1.15);
		}
		if (c.usingSpecial) {
		...
		if(c.prayerActive[25]) {
			...
			strength = (int)(strength * c.getCurse().getTurmoilMultiplier("Strength"));
	...
	public static boolean hasKorasi(Client c) {
		return  c.playerEquipment[c.playerWeapon] == 19784; 
	}
	
	public static boolean hasScimitar(Client c) {
		return c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).contains("scimitar"); 
	}
	...
	public static int calculateMeleeDefence(Client c) {
		...
		if (c.prayerActive[0]) {
			...
			defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.50;	
		} else if (c.curseActive[19]) {
			defenceLevel = (int)(defenceLevel * c.getCurse().getTurmoilMultiplier("Defence"));
        }
	...
	public static int calculateMeleeAttack(Client c) {
		...
		if (c.prayerActive[2]) {
			...
			attackLevel = (int)(attackLevel * c.getCurse().getTurmoilMultiplier("Attack"));
		}
		if (c.fullVoidMelee()) {
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1;
		}
		if (c.slayerHelmAffect) {
			attackLevel *= (double) 1.15;
		}
