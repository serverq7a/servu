package org.mistex.game.world.player.combat.range;
...
public class RangeData {
	...
	public static void fireProjectilePlayer(Client c) {
		if(c.oldPlayerIndex > 0) {
			if(World.players[c.oldPlayerIndex] != null) {
				c.projectileStage = 2;
				int pX = c.getX();
				int pY = c.getY();
				int oX = World.players[c.oldPlayerIndex].getX();
				int oY = World.players[c.oldPlayerIndex].getY();
				int offX = (pY - oY)* -1;
				int offY = (pX - oX)* -1;	
				if (!c.msbSpec)
