	...
	public static void playerAlching(Client c, int spell, int itemId, int slot) {
		switch (spell) {
			...
			if(System.currentTimeMillis() - c.alchDelay > 1000) {	
				...
				int reward = c.getItems().getItemShopValue(itemId)/3;
				...
				c.getPA().sendFrame106(6);
				c.getPA().addSkillXP(c.MAGIC_SPELLS[49][7] * PlayerConfiguration.MAGIC_EXP_RATE, 6);
			...
			if(System.currentTimeMillis() - c.alchDelay > 2000) {	
				...
				int reward = (int)(c.getItems().getItemShopValue(itemId)*.75);
				int playerAmount = c.getItems().getItemAmount(995);
				if(reward + playerAmount > 2147483647) {
				...
				c.alchDelay = System.currentTimeMillis();
				c.getPA().sendFrame106(6);
				c.getPA().addSkillXP(c.MAGIC_SPELLS[50][7] * PlayerConfiguration.MAGIC_EXP_RATE, 6);
				c.getPA().refreshSkill(6);
				}
				}
			}
