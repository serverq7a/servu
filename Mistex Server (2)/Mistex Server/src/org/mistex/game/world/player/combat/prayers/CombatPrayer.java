package org.mistex.game.world.player.combat.prayers;
...
public class CombatPrayer {
    ...
    public static void handlePrayerDrain(Client c) {
    	...
    	for (int j = 0; j < c.getCurse().curseData.length; j++) {
			if (c.curseActive[j]) {
				toRemove += c.getCurse().curseData[j]/20;
				c.usingPrayer = true;
			}
		}
        if (toRemove > 0) {
	...
	public static void activatePrayer(Client c, int i) {
		...
		if (c.inWeaponGame() || c.inWeaponLobby()) {
			c.sendMessage("You can not use prayers here!");
			return;
		}
		if (c.duelRule[7]) {
		...
		if(c.playerLevel[5] > 0 || !MistexConfiguration.PRAYER_POINTS_REQUIRED){
			if(c.getPA().getLevelForXP(c.playerXP[5]) >= c.PRAYER_LEVEL_REQUIRED[i] || !MistexConfiguration.PRAYER_LEVEL_REQUIRED) {
