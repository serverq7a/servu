...
public class BankPins {
	...
	public BankPins(Client client) {
		this.client = client;
	}

	/**
	 * Bank pins recovery delay
	 */
	...
	/**
	 * Date that player's bank pin expires
	 * @return
	 */
	...
	/**
	 * The date requested for bank pin cancellation 
	 * @return
	 */
	...
	public void deletePin() {
		firstPin = secondPin = thirdPin = fourthPin = client.playerBankPin = 0;
		falseButtons();
		client.enterdBankpin = false;
		client.hasBankPin = false;
		client.hasBankpin = false;
		client.sendMessage("@red@Your bank pin has been deleted.");
		client.getDH().sendDialogues(1273,494);
	}
	

	/**
	 * Bank pin setting frames
	 */
	...
	/**
	 * Resets the bank numbers on interface
	 * @return
	 */
	...
	public void bankPinEnter(int button) {
		if (allowTimer > 0 && allowTimer <= 300000) {
			...
			if (time >= 2) {
				client.sendMessage("@red@Please wait " + time+ " minutes before attempting your bank pin again.");
			} else if (time == 1) {
				client.sendMessage("@red@Please wait " + time+ " minute before attempting your bank pin again.");
			} else if (time <= 0) {
				client.sendMessage("@red@Please wait less "+ "than a minute before attempting your bank pin again.");
			}
			return;
		}
		sendPins();
	...
	/**
	 * Handles the first bank pin button
	 * @param button
	 */
	...
	/**
	 * Handles the second bank pin button
	 * @param button
	 */
	...
	/**
	 * Handles the third bank pin button
	 * @param button
	 */
	...
	/**
	 * Handles the fourth bank pin button
	 * @param button
	 */
	private void handleButtonFour(int button) {
		...
		if (client.bankPin1 == one && client.bankPin2 == two && client.bankPin3 == three && client.bankPin4 == four) {
			...
			if (client.attemptsRemaining > 1) {
				client.sendMessage("@red@Invalid pin. You have "+ client.attemptsRemaining + " attempts remaining.");
			} else if (client.attemptsRemaining == 1) {
				client.sendMessage("@red@Invalid pin. You have "+ client.attemptsRemaining + " attempt remaining.");
			} else if (client.attemptsRemaining <= 0) {
				client.sendMessage("@red@Invalid pin. You must wait 5 minutes before attempting again.");
			}
			client.getPA().removeAllWindows();
			falseButtons();
		}
	...
	/**
	 * Randomizes the bank pin numbers
	 */
	private void randomizeNumbers() {
		int i = MistexUtility.random(4);
	...
	private int stringIds[] = { 14883, 14884, 14885, 14886, 14887, 14888,14889, 14890, 14891, 14892 };
	...
	public String toString() {
		return "BankPin{" + "recovery_Delay=" + recovery_Delay + ", client="+ client + ", allowTimer=" + allowTimer + ", bankPins="
