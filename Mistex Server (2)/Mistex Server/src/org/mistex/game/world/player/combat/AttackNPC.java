...
import org.mistex.game.world.content.Attractor;
...
public class AttackNPC {
	...
	public static void addNPCHit(final int i, final Client c) {
		...
		if (!c.usingClaws) {
			if (c.projectileStage == 0 && !c.usingMagic) {
				...
				c.getCombat().applyNpcMeleeDamage(i, 1, MistexUtility.random(c.getCombat().calculateMeleeMaxHit()));
				if (c.doubleHit) {
					if (!c.oldSpec) {
						c.getCombat().applyNpcMeleeDamage(i, 2, MistexUtility.random(c.getCombat().calculateMeleeMaxHit()));
					} else {
	...
	public static boolean KQnpc(int i) {
		switch (Mistex.npcHandler.npcs[i].npcType) {
		case 1158:
			return true;
		}
		return false;
	...
	public static void applyNpcMeleeDamage(Client c, int i, int damageMask, int damage) {
		if (c.doubleHit && c.usingClaws) {
			...
			c.getCombat().applyNpcMeleeDamage(i, 1, c.clawDamage);
			c.getCombat().applyNpcMeleeDamage(i, 2, c.clawDamage / 2);
		}
		damage = MistexUtility.random(c.getCombat().calculateMeleeMaxHit());

		c.previousDamage = damage;

		boolean fullVeracsEffect = c.getPA().fullVeracs() && MistexUtility.random(3) == 1;
		...
		if (KQnpc(i) && !c.fullVerac() || c.usingMagic) {
			c.getCombat().resetPlayerAttack();
			c.sendMessage("Your attacks have no effect on the Queen.");
			return;
		}
		if (!fullVeracsEffect) {
			if (MistexUtility.random(NPCHandler.npcs[i].defence) > 10 + MistexUtility.random(c.getCombat().calculateMeleeAttack())) {
				damage = 0;
	...
	public static void delayedHit(final Client c, final int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if (NPCHandler.npcs[i].underAttackBy > 0 && Mistex.npcHandler.getsPulled(i)) {
				NPCHandler.npcs[i].killerId = c.playerId;
			} else if (NPCHandler.npcs[i].underAttackBy < 0 && !Mistex.npcHandler.getsPulled(i)) {
				NPCHandler.npcs[i].killerId = c.playerId;
			}
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (MistexUtility.random(NPCHandler.npcs[i].defence) > MistexUtility.random(10 + c.getCombat().calculateRangeAttack()) && !c.ignoreDefence) {
					damage = 0;
				} else if (NPCHandler.npcs[i].npcType == 2881 || NPCHandler.npcs[i].npcType == 2883 && !c.ignoreDefence) {
					damage = 0;
				}
				if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1) {
					if (MistexUtility.random(NPCHandler.npcs[i].defence) > MistexUtility.random(10 + c.getCombat().calculateRangeAttack()))
						damage2 = 0;
				}
				if (c.dbowSpec) {
				...
				c.dropArrow = !Attractor.activate(c);
				if (dropArrows && c.dropArrow) {
				...
				if (Mistex.npcHandler.getNPCs()[i].attackTimer < 9)
						...
						if (NPCHandler.npcs[j] != null && NPCHandler.npcs[j].MaxHP > 0) {
							...
							if ((nX - pX == -1 || nX - pX == 0 || nX - pX == 1) && (nY - pY == -1 || nY - pY == 0 || nY - pY == 1)) {
								if (NPCHandler.npcs[i].inMulti()) {
									Client p = (Client) World.players[c.playerId];
									c.getCombat().appendMutliChinchompa(j);
									Mistex.npcHandler.attackPlayer(p, j);
								}
							}
						}
					}
				...
				if (c.fullVoidMage() && c.playerEquipment[c.playerWeapon] == 8841) {
					damage = MistexUtility.random(c.getCombat().magicMaxHit() + 10);
				} else {
					damage = MistexUtility.random(c.getCombat().magicMaxHit());
				}
				if (c.getCombat().godSpells()) {
					if (System.currentTimeMillis() - c.godSpellDelay < MistexConfiguration.GOD_SPELL_CHARGE) {
						damage += MistexUtility.random(10);
					}
				}
				boolean magicFailed = false;
				if (MistexUtility.random(NPCHandler.npcs[i].defence) > 10 + MistexUtility.random(c.getCombat().mageAtk())) {
					damage = 0;
					magicFailed = true;
				...
				if (c.magicDef) {
					c.getPA().addSkillXP((damage * PlayerConfiguration.MELEE_EXP_RATE / 2), 1);
					c.getPA().refreshSkill(1);
				}
				...
				if (!magicFailed) {
					...
					case 12911:
					case 12929:
						int heal = MistexUtility.random(damage / 2);
	...
	public static void attackNpc(Client c, int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if (Mistex.npcHandler.checkSlayerHelm(c, i)) {
			...
			if (Mistex.npcHandler.npcs[i].summon == true) {
				if (Mistex.npcHandler.npcs[i].index != c.playerId || c.wildLevel == 0) {
					c.getCombat().resetPlayerAttack();
					c.sendMessage("You cannot attack this monster.");
					return;
				}
			}
			if (Mistex.npcHandler.npcs[i].inMulti() && c.lastsummon > 0) {
				Mistex.npcHandler.attackNPC(c, i, c.summoningnpcid);
			}
			if (!c.getCombat().goodSlayer(i)) {
				c.getCombat().resetPlayerAttack();
				return;
			}
			if (NPCHandler.npcs[i].spawnedBy != c.playerId && NPCHandler.npcs[i].spawnedBy > 0 && Mistex.npcHandler.npcs[i].summon != true) {
				c.getCombat().resetPlayerAttack();
			...
			if (c.attackTimer <= 0) {
				...
				if ((!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 2) && (c.getCombat().usingHally() && !c.usingOtherRangeWeapons && !c.usingBow && !c.usingMagic))
						|| (!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 4) && (c.usingOtherRangeWeapons && !c.usingBow && !c.usingMagic))
						|| (!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 1) && (!c.usingOtherRangeWeapons && !c.getCombat().usingHally() && !c.usingBow && !c.usingMagic)) || ((!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 8) && (c.usingBow || c.usingMagic)))) {
					c.attackTimer = 2;
					return;
				}
				...
				if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows] && MistexConfiguration.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 18357) {
				...
				if (c.soulSplitDelay <= 0) {
					c.getCombat().applynpcSoulSplit(i, 1);
				}

				if (c.usingBow && MistexConfiguration.CRYSTAL_BOW_DEGRADES) { // crystal
																				// bow
																				// degrading
