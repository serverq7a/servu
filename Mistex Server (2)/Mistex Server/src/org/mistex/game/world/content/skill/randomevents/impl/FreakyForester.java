...
import org.mistex.game.world.content.skill.randomevents.EventHandler;
...
public class FreakyForester extends EventHandler {
	...
	public static String getPheasant(Client c) {
		if (c.getPheasent < 0) {
			c.getPheasent = MistexUtility.random(3);
		}
	...
	public static void leaveArea(Client c) {
		if (c.killedPheasant[c.getPheasent]) {
			...
			c.getItems().addItem(6180 + MistexUtility.random(2), 1);
		} else {
			failEvent(c);
		}
		for (int i = 0; i < 4; i++) {
