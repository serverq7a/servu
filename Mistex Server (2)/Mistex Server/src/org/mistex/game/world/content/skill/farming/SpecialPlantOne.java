...
public class SpecialPlantOne {
	...
	public SpecialPlantOne(Client player) {
		this.player = player;
	}
	...
	public enum SpecialPlantFieldsData {
		BRIMHAVEN(0, new Point[]{new Point(2801, 3202), new Point(2803, 3204)}, 5375), KARAMJA(1, new Point[]{new Point(2795, 3100), new Point(2797, 3102)}, 5503), DRAYNOR(2, new Point[]{new Point(3059, 3257), new Point(3061, 3259)}, 5375), ETCETERIA(3, new Point[]{new Point(2612, 3857), new Point(2614, 3859)}, 5375);
	...
	public void doCalculations() {
		for (int i = 0; i < specialPlantSaplings.length; i++) {
			if (specialPlantStages[i] > 0 && specialPlantStages[i] <= 3 && Mistex.getMinutesCounter() - specialPlantTimer[i] >= 5) {
				specialPlantStages[i]--;
				specialPlantTimer[i] = Mistex.getMinutesCounter();
				updateSpecialPlants();
				continue;
			}
			...
			long difference = Mistex.getMinutesCounter() - specialPlantTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				specialPlantTimer[hopsFieldsData.getSpecialPlantsIndex()] = Mistex.getMinutesCounter();
				updateSpecialPlants();
				if (specialPlantStages[hopsFieldsData.getSpecialPlantsIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (specialPlantData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + specialPlantData.getLevelRequired() + " to plant this sapling.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				specialPlantTimer[specialPlantFieldData.getSpecialPlantsIndex()] = Mistex.getMinutesCounter();
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
					...
					specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Mistex.getMinutesCounter() - specialPlantData.getGrowthTime();
					modifyStage(specialPlantFieldsData.getSpecialPlantsIndex());
					container.stop();
					return;
				}
				
				player.sendMessage("You harvest the crop, and pick some " + ItemAssistant.getItemName(specialPlantData.getHarvestId()).toLowerCase() + ".");
...
//		SpiritTree.sendDialogue(player, 3636);
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] == 1) {
			...
			player.getDH().sendStatement("This plant has fully grown. You can check it's health", "to gain some farming experiences.");
			return true;
		}
		if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 0) {
			player.getDH().sendStatement("This is one of the special patches. The soil has not been treated.", "The patch needs weeding.");
		} else if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
			player.getDH().sendStatement("This is one of the special patches. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4]);
