...
import org.mistex.game.world.content.skill.farming.ChopTree.Tree;
...
public class WoodTrees {
	...
	public WoodTrees(Client player) {
		this.player = player;
	}
	...
	public enum TreeFieldsData {
		VARROCK(0, new Point[]{new Point(3228, 3458), new Point(3230, 3460)}, 2341), LUMBRIDGE(1, new Point[]{new Point(3192, 3230), new Point(3194, 3232)}, 2342), TAVERLEY(2, new Point[]{new Point(2935, 3437), new Point(2937, 3439)}, 2339), FALADOR(3, new Point[]{new Point(3003, 3372), new Point(3005, 3374)}, 2340);
	...
	public void doCalculations() {
		for (int i = 0; i < treeSaplings.length; i++) {
			if (treeStages[i] > 0 && treeStages[i] <= 3 && Mistex.getMinutesCounter() - treeTimer[i] >= 5) {
				treeStages[i]--;
				treeTimer[i] = Mistex.getMinutesCounter();
				updateTreeStates();
				continue;
			}
			...
			long difference = Mistex.getMinutesCounter() - treeTimer[i];
	...
	public void doStateCalculation(int index) {
		...
		if (treeState[index] == 0 && treeStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			if (MistexUtility.random(100) <= maxChance) {
				treeState[index] = 1;
			}
		}
	}
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				treeTimer[hopsFieldsData.getTreeIndex()] = Mistex.getMinutesCounter();
				updateTreeStates();
				if (treeStages[hopsFieldsData.getTreeIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (treeData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + treeData.getLevelRequired() + " to plant this sapling.");
			return true;
		}

		if (!player.getItems().playerHasItem(FarmingConstants.TROWEL)) {
			player.getDH().sendStatement("You need a trowel to plant the sapling here.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				treeTimer[treeFieldsData.getTreeIndex()] = Mistex.getMinutesCounter();
	...
	public boolean checkHealth(int objectX, int objectY) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.sendMessage("You examine the tree for signs of disease and find that it is in perfect health");
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (treeState[treeFieldsData.getTreeIndex()] == 1) {
			player.getDH().sendStatement("This tree is diseased. Use secateurs to prune the area, ", "or clear the patch with a spade.");
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 2) {
			player.getDH().sendStatement("This tree is dead. You did not cure it while it was diseased.", "Clear the patch with a spade.");
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 6) {
			player.getDH().sendStatement("This is a tree stump, to remove it, use a spade on it", "to recieve some roots and clear the patch.");
			return true;
		}
		if (treeStages[treeFieldsData.getTreeIndex()] == 0) {
			player.getDH().sendStatement("This is a tree patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (treeStages[treeFieldsData.getTreeIndex()] == 3) {
			player.getDH().sendStatement("This is a tree patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (treeStages[treeFieldsData.getTreeIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[treeStages[treeFieldsData.getTreeIndex()] - 4]);
	...
	public boolean cut(final int x, final int y) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.isSkilling || player.getItems().freeSlots() <= 0) {
					container.stop();
					return;
				}
				
				player.startAnimation(emoteId);
				if (SkillHandler.skillCheck(player.playerLevel[player.playerWoodcutting], treeLevel, axeChance*20)) {
					player.getItems().addItem(ChopTree.getTree(object).getLog(), 1);
					player.sendMessage("You get some " + ItemAssistant.getItemName(ChopTree.getTree(object).getLog()).toLowerCase() + ".");
					player.getPA().addSkillXP((int) ChopTree.getTree(object).getXP(), player.playerWoodcutting);
					if(MistexUtility.random(100) <= tree.getDecayChance()) {
	...
	public boolean canCut(final int x, final int y) {
		...
		if (player.getItems().freeSlots() + 1 >= 28) {
			return false;
		}
		if (!SkillHandler.hasRequiredLevel(player, player.playerWoodcutting, ChopTree.getTree(object).getLevel(), "woodcutting", "chop this tree")) {
			return false;
		}
		return true;
	}
