...
public class Implings {
	...
	private static enum Imps {
		...
		BABY_IMPLING(1028, 10, 25, 11238), 
		YOUNG_IMPLING(1029, 20, 65, 11240), 
		GOURMET_IMPLING(1030, 30, 113, 11242), 
		EARTH_IMPLING(1031, 40, 177, 11244), 
		ESSENCE_IMPLING(1032, 50, 255, 11246), 
		ECLECTIC_IMPLING(1033, 60, 289, 11248), 
		NATURE_IMPLING(1034, 70, 300, 11250),	
		MAGPIE_IMPLING(1035, 85, 400, 11252), 
		NINJA_IMPLING(6063, 80, 500, 11254),
		DRAGON_IMPLING(6064, 95, 600, 11256);
	...
	public static void catchImp(final Client c, final int ID, final int impId) {
		...
		if (MistexUtility.random(10) >= ((c.playerLevel[Constants.HUNTER] - 10) / 10) + 1) {
		...
		new ButterflyCatching().giveItem(c, i.getJar(), 1, "You catch the " + i.getName() + "!");
		new ButterflyCatching().addExp(c, Constants.hasMagicNet(c) ? i.getXP() * 2 : i.getXP());
	...
	private final static int[] magpieLoots = { 995, 1712, 892, 861, 1428,2437 };
	private final static int[] magpieAmount = { 2016, 1, 45, 1, 1, 3 };

	private final static int[] ninjaLoots = { 995, 2481, 5300, 1334, 44, 53,1085 };
	private final static int[] ninjaAmount = { 2733, 1, 1, 1, 20, 40, 1 };

	private final static int[] dragonLoots = { 995, 9244, 1725, 3105, 1319, 1215};
	private final static int[] dragonAmount = { 5000, 10, 1, 1, 1, 1};
	...
	public static void lootJar(final Client c, final int jar) {
		...
		switch (jar) {
		case 11238:
			loot = (MistexUtility.random(babyLoots.length - 1));
			type = babyLoots;
			amount = babyAmount;
			break;
		case 11240:
			loot = (MistexUtility.random(youngLoots.length - 1));
			type = youngLoots;
			amount = youngAmount;
			break;
		case 11242:
			loot = (MistexUtility.random(gormLoots.length - 1));
			type = gormLoots;
			amount = gormAmount;
			break;
		case 11244:
			loot = (MistexUtility.random(earthLoots.length - 1));
			type = earthLoots;
			amount = earthAmount;
			break;
		case 11246:
			loot = (MistexUtility.random(essLoots.length - 1));
			type = essLoots;
			amount = essAmount;
			break;
		case 11248:
			loot = (MistexUtility.random(eclecticLoots.length - 1));
			type = eclecticLoots;
			amount = eclecticAmount;
			break;
		case 11250:
			loot = (MistexUtility.random(natureLoots.length - 1));
			type = natureLoots;
			amount = natureAmount;
			break;
		case 11252:
			loot = (MistexUtility.random(magpieLoots.length - 1));
			type = magpieLoots;
			amount = magpieAmount;
			break;
		case 11254:
			loot = (MistexUtility.random(ninjaLoots.length - 1));
			type = ninjaLoots;
			amount = ninjaAmount;
			break;
		case 11256:
			loot = (MistexUtility.random(dragonLoots.length - 1));
			type = dragonLoots;
			amount = dragonAmount;
			break;
		}
