...
public class SpecialPlantTwo {
	...
	public SpecialPlantTwo(Client player) {
		this.player = player;
	}
	...
	public enum SpecialPlantFieldsData {
		DRAYNOR_MANOR(0, new Point[]{new Point(3086, 3354), new Point(3087, 3355)}, 5281), AL_KARID(2, new Point[]{new Point(3315, 3202), new Point(3316, 3203)}, 5280), CANIFIS(3, new Point[]{new Point(3451, 3472), new Point(3452, 3473)}, 5282);
	...
	public boolean plantSeeds(int objectX, int objectY, final int seedId) {
		...
		if (specialPlantData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + specialPlantData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (player.getItems().getItemAmount(specialPlantData.getSeedId()) < specialPlantData.getSeedAmount()) {
			player.getDH().sendStatement("You need atleast " + specialPlantData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Mistex.getMinutesCounter();
