...
public class Food {
	  ...
	  public boolean hasTorva() {
	    return c.playerEquipment[c.playerHat] == 20135 && c.playerEquipment[c.playerChest] == 20439 && c.playerEquipment[c.playerLegs] == 20143;
	  }
		...
		public void eat(int id, int slot) {
			...
			if (System.currentTimeMillis() - c.foodDelay >= 1500 && c.playerLevel[3] > 0) {
	        	...
	        	c.foodEaten += 1;
				if (c.foodEaten == 1)
					c.sendMessage("@war@You have started the achievement: Munchies ");
					InterfaceText.writeText(new AchievementTab(c));
				if (c.foodEaten == 1000)
					AchievementHandler.activateAchievement(c, AchievementList.MUNCHIES);
	        }
	    }
