...
public class FruitTree {
	...
	public FruitTree(Client player) {
		this.player = player;
	}
	...
	public void doCalculations() {
		for (int i = 0; i < fruitTreeSaplings.length; i++) {
			if (fruitTreeStages[i] > 0 && fruitTreeStages[i] <= 3 && Mistex.getMinutesCounter() - fruitTreeTimer[i] >= 5) {
				fruitTreeStages[i]--;
				fruitTreeTimer[i] = Mistex.getMinutesCounter();
			...
			long difference = Mistex.getMinutesCounter() - fruitTreeTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Mistex.getMinutesCounter();
				updateFruitTreeStates();
				if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (fruitTreeData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + fruitTreeData.getLevelRequired() + " to plant this sapling.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
					...
					fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Mistex.getMinutesCounter() - fruitTreeData.getGrowthTime();
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] == 1) {
			...
			player.getDH().sendStatement("This is a fruit tree stump, to remove it, use a ", "spade on it to clear the patch");
			return true;
		}
		if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 0) {
			player.getDH().sendStatement("This is a fruit tree patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
			player.getDH().sendStatement("This is a fruit tree patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4]);
