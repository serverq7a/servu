...
public class Allotments {
	...
	public enum InspectData {
		...
		WATERMELONS(5321, new String[][]{{"The strawberry seeds have only just been planted."}, {"The strawberry plants have more leaves than before."}, {"The strawberry plants have even more leaves and is", "slightly taller than before."}, {"Each strawberry plant has opened one white", "flower each."}, {"The strawberry plants are slightly larger, and", "have small strawberries visible at their bases."}, {"The strawberry plants are slightly larger, opened", "a second flower each, and have more strawberries."}, {"The strawberry plants are ready to harvest. The", "strawberries are almost as large as the flowers."}});
	...
	public void doCalculations() {
		for (int i = 0; i < allotmentSeeds.length; i++) {
			if (allotmentStages[i] > 0 && allotmentStages[i] <= 3 && Mistex.getMinutesCounter() - allotmentTimer[i] >= 5) {
				allotmentStages[i]--;
				allotmentTimer[i] = Mistex.getMinutesCounter();
			...
			long difference = Mistex.getMinutesCounter() - allotmentTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				allotmentTimer[allotmentFieldsData.getAllotmentIndex()] = Mistex.getMinutesCounter();
				updateAllotmentsStates();
				if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 3) {
					container.stop();
					return;
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] == 0) {
					allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] = (int) (1 + (START_HARVEST_AMOUNT + MistexUtility.random((END_HARVEST_AMOUNT + (player.playerEquipment[player.playerWeapon] == 7409 ? 15 : 0)) - START_HARVEST_AMOUNT)) * (1));
				}
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
...
//		player.getSkillGuide().selected = 20;
	...
	public void handleFlowerProtection(int index) {
		...
		if (!allotmentWatched[index]) {// Misc.random(100) <= maxChance) {
			...
			if (player.getFlowers().flowerSeeds[indexGiven] >= 0x21 && player.getFlowers().flowerSeeds[indexGiven] <= 0x24) {
				if (allotmentData.getFlowerProtect() == Flowers.SCARECROW) {
					return;
				}
			}
			if (player.getFlowers().flowerState[indexGiven] != 3 && player.getFlowers().hasFullyGrown[indexGiven] && player.getFlowers().flowerSeeds[indexGiven] == allotmentData.getFlowerProtect()) {
				player.getFlowers().flowerState[indexGiven] = 3;
				player.getFlowers().updateFlowerStates();
			} else if (MistexUtility.random(100) <= maxChance){
//				allotmentState[index] = 2;
//				player.sendMessage("One of your crops is diseased!");
			}
		}
