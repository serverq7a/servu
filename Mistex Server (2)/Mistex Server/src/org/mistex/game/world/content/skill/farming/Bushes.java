...
public class Bushes {
	...
	public void doCalculations() {
		for (int i = 0; i < bushesSeeds.length; i++) {
			if (bushesStages[i] > 0 && bushesStages[i] <= 3 && Mistex.getMinutesCounter() - bushesTimer[i] >= 5) {
				bushesStages[i]--;
				bushesTimer[i] = Mistex.getMinutesCounter();
				updateBushesStates();
				continue;
			}
			...
			long difference = Mistex.getMinutesCounter() - bushesTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				bushesTimer[bushesFieldsData.getBushesIndex()] = Mistex.getMinutesCounter();
				updateBushesStates();
				if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (bushesData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + bushesData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (player.getItems().getItemAmount(bushesData.getSeedId()) < bushesData.getSeedAmount()) {
			player.getDH().sendStatement("You need atleast " + bushesData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (bushesState[bushesFieldsData.getBushesIndex()] == 3) {
					...
					bushesTimer[bushesFieldsData.getBushesIndex()] = Mistex.getMinutesCounter() - bushesData.getGrowthTime();
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (bushesStages[bushesFieldsData.getBushesIndex()] == 0) {
			player.getDH().sendStatement("This is a bush patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
			player.getDH().sendStatement("This is a bush patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (bushesStages[bushesFieldsData.getBushesIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[bushesStages[bushesFieldsData.getBushesIndex()] - 4]);
