...
public class ReportHandler {
	...
	public static void addText(String name, byte[] data, int dataLength) {
		...
		savedNames[0] = name;
		savedSpeach[0] = MistexUtility.textUnpack(data, dataLength);
	...
	public static void handleReport(Client c) throws Exception {
		String player = MistexUtility.longToReportPlayerName(c.inStream.readQWord2());
		...
		if (hasSpoke(player)) {
			...
			if (c.playerReported == false)
				AchievementHandler.activateAchievement(c, AchievementList.LAW_ABIDING_CITIZEN);
			return;
		} else {
