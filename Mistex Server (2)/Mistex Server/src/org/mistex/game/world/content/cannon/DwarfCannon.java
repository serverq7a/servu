...
public class DwarfCannon {
	...
	public static boolean canSetupCannon(Client player) {
        int x = player.getX();
        int y = player.getY();
        int z = player.heightLevel;
        if (x >= 2502 && y >= 2820 && x <= 2614 && y <= 2866) {
            return false;
        }
        return true;
    }
    ...
    public void setup() {
        ...
        if (player.getItems().playerHasItem(6, 1) && player.getItems().playerHasItem(8, 1) && player.getItems().playerHasItem(10, 1) && player.getItems().playerHasItem(12, 1)) {
            ...
            Mistex.objectHandler.addObject(base);
            player.getPA().object(base.getObjectId(), base.getObjectX(), base.getObjectY(), 0, 10);
            Mistex.objectHandler.placeObject(base);
				...
				public void execute(CycleEventContainer p) {
					 if (player.disconnected) {
					  ...
					  player.startAnimation(827);
	                    ...
	                    Mistex.objectHandler.removeObject(cannon);
	                    Mistex.objectHandler.addObject(stand);
	                    player.getPA().object(stand.getObjectId(), stand.getObjectX(), stand.getObjectY(), 0, 10);
	                    Mistex.objectHandler.placeObject(stand);
            ...
            CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
        				public void execute(CycleEventContainer p) {
        					 ...
        					 player.startAnimation(827);
        	                    ...
        	                    Mistex.objectHandler.removeObject(cannon);
        	                    Mistex.objectHandler.addObject(barrel);
        	                    player.getPA().object(barrel.getObjectId(), barrel.getObjectX(), barrel.getObjectY(), 0, 10);
        	                    Mistex.objectHandler.placeObject(barrel);
        				...
        				public void execute(CycleEventContainer p) {
        					 if (player.disconnected) {
        					   ...
        					   player.startAnimation(827);
        	                    ...
        	                    Mistex.objectHandler.removeObject(cannon);
        	                    Mistex.objectHandler.addObject(newCannon);
        	                    player.getPA().object(newCannon.getObjectId(), newCannon.getObjectX(), newCannon.getObjectY(), 0, 10);
        	                    Mistex.objectHandler.placeObject(newCannon);
    ...
    private void degrade() {
    	CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
    		public void execute(CycleEventContainer p) {
    			player.sendMessage("Your cannon has almost decayed");
    			p.stop();
    		}
    	...
    	CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
    		public void execute(CycleEventContainer p) {
    			...
    			player.sendMessage("Your cannon has decayed.");
                player.getPA().object(0, cannon.getPosition().getX(), cannon.getPosition().getY(), 0, 10);
                Mistex.objectHandler.removeObject(cannon);
                remove();
                cannon = null;
    ...
    public void giveBalls(Position position) {
        if (balls == 30) {
            player.sendMessage("You can only hold 30 balls at a time");
            return;
        }
        if (player.getItems().playerHasItem(2)) {
            int amountToBeGiven = player.getItems().getItemCount(2) > 30 ? (30 - balls) : (30 - player.getItems().getItemCount(2));
            balls += amountToBeGiven;
            player.getItems().deleteItem(2, amountToBeGiven);
            shoot(position);
            player.sendMessage("You put the balls into the cannon.");
        }
    }
    ...
    public void pickup(Position p) {
        ...
        if (cannon.getPosition().getX() == p.getX() && cannon.getPosition().getY() == p.getY() && cannon.getPosition().getZ() == p.getZ()) {
            ...
            shooting = false;
            player.startAnimation(827);
            /*Objects empty = new Objects(6951, new Position(cannon.getPosition().getX(), cannon.getPosition().getY(), cannon.getPosition().getZ()), 0, 10, 0);
            ...
            if (player.getItems().freeSlots() > 3) {
                ...
                player.getBank().addItem(7, 1);
                player.getBank().addItem(9, 1);
                player.getBank().addItem(11, 1);
                player.getBank().addItem(13, 1);
            ...
            if (balls > 0) {
                if (player.getItems().freeSlots() > 0) {
                    ...
                    player.getBank().addItem(3, balls);
    ...
    private void fireAtMobs() {
         ...
         */
        int damage = MistexUtility.random(MAX_DAMAGE);
        ...
        if (hit != null) {
            if (player.inMulti() && player.inWild()) {
                ...
                hit.hitUpdateRequired2 = true;
                hit.killerId = player.getId();
            } else {
                if (hit.underAttack && hit.killerId != player.getId())
                    return;
     ...
     */
    private void cannonProjectile(Client player, Objects cannon, NPC n) {
