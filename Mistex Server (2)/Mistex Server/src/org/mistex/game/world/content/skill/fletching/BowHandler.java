...
public class BowHandler {
	...
	private static enum Bow {
		...
		SHORTBOW(1511, 34205, 34204, 34203, 34202, 50, 5, 1),
		OAK_SHORTBOW(1521, 34185, 34184, 34183, 34182, 54, 17, 20),
		WILLOW_SHORTBOW(1519, 34185, 34184, 34183, 34182, 60, 33, 35),
		MAPLE_SHORTBOW(1517, 34185, 34184, 34183, 34182, 64, 50, 50),
		YEW_SHORTBOW(1515, 34185, 34184, 34183, 34182, 68, 68, 65),
		MAGIC_SHORTBOW(1513, 34170, 34169, 34168, 34167, 72, 83, 80);

		private int log, button, button5, button10, buttonAll, item, XP, req;

		private Bow(final int log, final int button, final int button5, final int button10, final int buttonAll, final int item, final int XP, final int req) {
			this.log = log;
			this.button = button;
	...
	private static enum Crossbow {
		...
		CROSSBOW(1511, 34213, 34212, 34211, 34210, 9440, 6, 9), OAK_CROSSBOW(
				1521, 34193, 34192, 34191, 34190, 9442, 22, 39), WILLOW_CROSSBOW(
				1519, 34193, 34192, 34191, 34190, 9444, 27, 46), MAPLE_CROSSBOW(
				1517, 34193, 34192, 34191, 34190, 9448, 41, 54), YEW_CROSSBOW(
				1515, 34193, 34192, 34191, 34190, 9452, 50, 69);
	...
	private static enum Longbow {
		...
		LONGBOW(1511, 34209, 34208, 34207, 34206, 48, 10, 10), OAK_LONGBOW(
				...
				1513, 34174, 34173, 34172, 34171, 70, 92, 85);
	...
	public static void fletchCrossbow(final Client c, final int button) {
		final Crossbow l = Crossbow.itemID(c.logID);
		if (l == null) {
			return;
		}
		if (button == l.getButton12() || button == l.getButton52() || button == l.getButton102() || button == l.getButtonAll2()) {
			if (c.playerSkilling[Player.playerFletching] == true) {
				return;
			}
			if (c.playerLevel[Player.playerFletching] < l.getReq2()) {
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					new ArrowHandler().giveItem(c, l.getBow2(), 1, null);
					new ArrowHandler().addExp(c, l.getXP2());
					c.doAmount--;
				}
	...
	public static void fletchUnstrungLongbow(final Client c, final int button) {
		...
		if (button == l.getButton11() || button == l.getButton51()
			...
			if (c.playerLevel[Player.playerFletching] < l.getReq1()) {
				c.sendMessage("You must have a fletching level of at least "
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					c.startAnimation(ANIMATION);
					c.getItems().deleteItem(c.logID, 1);
					new ArrowHandler().giveItem(c, l.getBow1(), 1, null);
					new ArrowHandler().addExp(c, l.getXP1());
					c.doAmount--;
				}
	...
	public static void fletchUnstrungBow(final Client c, final int button) {
		...
		if (button == b.getButton1() || button == b.getButton5()
			...
			if (c.playerLevel[Player.playerFletching] < b.getReq()) {
				c.sendMessage("You must have a fletching level of at least "
						+ b.getReq() + " to do this.");
				return;
			}
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					c.startAnimation(ANIMATION);
					c.getItems().deleteItem(c.logID, 1);
					new ArrowHandler().giveItem(c, b.getBow(), 1, null);
					new ArrowHandler().addExp(c, b.getXP());
					c.doAmount--;
				}
	...
	public static void handleInterface(final Client c, final int useWith, final int itemUsed) {
		if (c.isSkilling) {
			return;
		}
	...
	public static void handleFletchingButtons(final Client c, final int button) {
		if (c.playerSkilling[Player.playerFletching] == true) {
			return;
		}
		if (!c.getItems().playerHasItem(KNIFE)
	...
	private static void fletchShaft(final Client c, final int button) {
		int item = -1;
		switch (button) {
		case 34217:
			...
			if (c.getItems().playerHasItem(KNIFE)
				...
				new ArrowHandler().giveItem(c, 52, AMOUNT, null);
				new ArrowHandler().addExp(c, 15);
				c.getPA().closeAllWindows();
				c.logID = -1;
			} else {
				c.sendMessage("You do not have the correct supplies to fletch this.");
	...
	private static void openFletching(final Client c, final int item) {
		if (item == 1511) {
	...
	private static void openFletching1(final Client c, final int item) {
		if (item == 1521) {
	...
	private static void openFletching2(final Client c, final int item) {
		if (item == 1515) {
			c.getPA().sendFrame164(8880);
			c.getPA().sendFrame126("What would you like to make?", 8879);
	...
	private static void openFletching3(final Client c, final int item) {
		if (item == 1519) {
			c.getPA().sendFrame164(8880);
			c.getPA().sendFrame126("What would you like to make?", 8879);
	...
	private static void openFletching4(final Client c, final int item) {
		if (item == 1517) {
			c.getPA().sendFrame164(8880);
			c.getPA().sendFrame126("What would you like to make?", 8879);
	...
	private static void openFletching5(final Client c, final int item) {
		if (item == 1513) {
