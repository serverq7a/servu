...
public class Barrows {
	...
	public void checkCoffins() {
			if (c.barrowsKillCount < 5) {
				...
				Mistex.npcHandler.spawnNpc(c, 2026, c.getX(), c.getY()-1, 3, 0, 120, 25, 200, 200, true, true);
				c.getPA().removeAllWindows();
			} else if (c.barrowsKillCount > 5) {
	 ...
	 */
	public void reward() {
		c.getItems().addItem(randomRunes(), MistexUtility.random(150) + 100);
		c.getItems().addItem(randomRunes(), MistexUtility.random(150) + 100);
	...
	public void useChest() {
		...
		if (c.barrowsKillCount == 5) {
			if (c.barrowsNpcs[4][1] == 0) {
				Mistex.npcHandler.spawnNpc(c, 2026, c.getX(), c.getY()-1, 0, 0, 120, 25, 200, 200, true, true);
			}
			c.barrowsNpcs[4][1] = 1;
		}
		if (c.barrowsKillCount > 5 && checkBarrows()) {
			if (c.getItems().freeSlots() >= 4) {
				reward();
				resetBarrows();
				int canTryMinigame = MistexUtility.random(4);
				if (canTryMinigame == 0) {
					cantWalk = true;
					challengeMinigame();
				}
