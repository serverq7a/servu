...
public class Flowers { // todo scarecrow 6059
	...
	public void doCalculations() {
		for (int i = 0; i < flowerSeeds.length; i++) {
			if (flowerStages[i] > 0 && flowerStages[i] <= 3 && Mistex.getMinutesCounter() - flowerTimer[i] >= 5) {
				flowerStages[i]--;
				flowerTimer[i] = Mistex.getMinutesCounter();
				updateFlowerStates();
			}
			if (Mistex.getMinutesCounter() - flowerTimer[i] >= 5 && flowerSeeds[i] > 0x21 && flowerSeeds[i] <= 0x24) {
				flowerSeeds[i]--;
				updateFlowerStates();
				return;
			}
			...
			long difference = Mistex.getMinutesCounter() - flowerTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				flowerTimer[flowerFieldsData.getFlowerIndex()] = Mistex.getMinutesCounter();
				updateFlowerStates();
				if (flowerStages[flowerFieldsData.getFlowerIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (flowerStages[flowerFieldsData.getFlowerIndex()] != 3) {
			player.getDH().sendStatement("You can't plant a seed here.");
			return false;
		}
		if (flowerData.getLevelRequired() > player.playerLevel[Player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + flowerData.getLevelRequired() + " to plant this seed.");
			return true;
		}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (flowerStages[flowerFieldsData.getFlowerIndex()] == 0) {
			player.getDH().sendStatement("This is an flower patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (flowerStages[flowerFieldsData.getFlowerIndex()] == 3) {
			player.getDH().sendStatement("This is an flower patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (flowerStages[flowerFieldsData.getFlowerIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[flowerStages[flowerFieldsData.getFlowerIndex()] - 4]);
