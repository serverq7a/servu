...
public class Compost {
	...
	public enum CompostBinLocations {
		NORTH_ARDOUGNE(0, FIRST_TYPE_COMPOST_BIN, 3, 2661, 3375), PHASMATYS(1, SECOND_TYPE_COMPOST_BIN, 1, 3610, 3522), FALADOR(2, FIRST_TYPE_COMPOST_BIN, 4, 3056, 3312), CATHERBY(3, FIRST_TYPE_COMPOST_BIN, 3, 2804, 3464);
	...
	private void updateCompostBin(int index) {
		...
		ObjectHandler.createAnObject(player, finalObject, x, y, CompostBinLocations.forId(index).getObjectFace());
	}
	...
	public void closeCompostBin(final int index) {
		...
		compostBinsTimer[index] = Mistex.getMinutesCounter();
	...
	public void openCompostBin(final int index) {
		...
		if (Mistex.getMinutesCounter() - compostBinsTimer[index] >= timerRequired) {
	...
	public void fillCompostBin(int x, int y, final int organicItemUsed) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.isSkilling || !player.getItems().playerHasItem(organicItemUsed) || organicItemAdded[index] == 15) {
					container.stop();
					return;
				}
	...
	public void retrieveCompost(final int index) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.isSkilling || !player.getItems().playerHasItem(BUCKET) && compostBins[index] != 350 || organicItemAdded[index] == 0) {
					container.stop();
					stop();
					return;
				}
				player.getPA().addSkillXP((int) (finalItem == COMPOST ? COMPOST_EXP_RETRIEVE : finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE), player.playerFarming);
