...
public class Herbs {
	...
	public enum HerbData {
		...
		public double getPlantingXp() {
			return plantingXp * Skill.Skills.FARMING.getMultiplier();
		}

		public double getHarvestXp() {
			return harvestXp * Skill.Skills.FARMING.getMultiplier();
		}
	...
	public void doCalculations() {
		for (int i = 0; i < herbSeeds.length; i++) {
			if (herbStages[i] > 0 && herbStages[i] <= 3 && Mistex.getMinutesCounter() - herbTimer[i] >= 1) {
				herbStages[i]-=2;
				herbTimer[i] = Mistex.getMinutesCounter();
				updateHerbsStates();
			}
			...
			long difference = Mistex.getMinutesCounter() - herbTimer[i];
...
//				herbState[index] = 1;
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				herbTimer[herbFieldsData.getHerbIndex()] = Mistex.getMinutesCounter();
				updateHerbsStates();
				if (herbStages[herbFieldsData.getHerbIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (herbData.getLevelRequired() > player.playerLevel[Player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + herbData.getLevelRequired() + " to plant this seed.");
			return true;
		}
	...
	public boolean harvest(int objectX, int objectY) {
		...
		player.startAnimation(FarmingConstants.PICKING_VEGETABLE_ANIM);
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {

			public void execute(CycleEventContainer container) {
				if (herbHarvest[herbFieldsData.getHerbIndex()] == 0) {
					herbHarvest[herbFieldsData.getHerbIndex()] = (int) (1 + (START_HARVEST_AMOUNT + MistexUtility.random((END_HARVEST_AMOUNT + (player.playerEquipment[player.playerWeapon] == 7409 ? 5 : 0)) - START_HARVEST_AMOUNT)) * (1));
				}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (herbStages[herbFieldsData.getHerbIndex()] == 0) {
			player.getDH().sendStatement("This is an herb patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (herbStages[herbFieldsData.getHerbIndex()] == 3) {
			player.getDH().sendStatement("This is an herb patch. The soil has not been treated.", "The patch is empty and weeded.");
...
//						player.getDH().sendStatement(inspectData.getMessages()[herbStages[herbFieldsData.getHerbIndex()] - 4]);
