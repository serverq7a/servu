...
public class PvPDrops {
	...
	static int [][] LOW_DROPS = {{11118,0},{554,15}, {557, 15}, {555,9}, {556,10}, {558,20}, {559,12},
								{560,9}, {561, 10}, {562,20}, {563,15}, {564, 12}, {565, 5}, {1073, 0}, 
								{1091, 0}, {1111, 0}, {1123, 0}, {1145, 0}, {1161,0}, {1183, 0}, {1199, 0},
								{1211,1},{1317, 0}, {1430,0}, {1185, 0}, {1213, 0},{1147, 0}};
	 ...
	 */
	static int [][] FOOD_DROPS = {{379,4}, {385,2}, {391, 2}, {373, 2}, {7946, 2}};
	...
	static void createDrop(Client player, Client o, int item, int [][] array) {
		...
		for (int i = 0; i < array.length; i++)
			if (array[i][0] == item)
				amount = 1 + MistexUtility.random(array[i][1]);
		Mistex.itemHandler.createGroundItem(o, item, player.getX(), player.getY(), amount, o.playerId);
		o.dropWealth += player.getItems().getItemShopValue(item) * amount;
	}
	...
	public static void handleDrops(Client player) {
		Client o = (Client) World.players[player.killerId];
		if (o != null) {
			/*
			 * Drops 1 item from inventory
			 */
			for(int i = 0; i < player.playerItems.length; i++) {
				if (1 + MistexUtility.random(27) == i && !player.getItems().tradeable(player.playerItems[i] - 1)) {
					Mistex.itemHandler.createGroundItem(o, player.playerItems[i] - 1, player.getX(), player.getY(), player.playerItemsN[i], o.playerId);
					o.dropWealth += player.getItems().getItemShopValue(player.playerItems[i] - 1) * player.playerItemsN[i];
				}
			}
			/*
			...
			for (int i = 0; i < player.playerEquipment.length; i++) {
				if (1 + MistexUtility.random(13) == i && !player.getItems().tradeable(player.playerEquipment[i])) {
					Mistex.itemHandler.createGroundItem(o, player.playerEquipment[i], player.getX(), player.getY(), player.playerEquipmentN[i], o.playerId);
					o.dropWealth += player.getItems().getItemShopValue(player.playerEquipment[i]) * player.playerEquipmentN[i];
				}
			}
			/*
			...
			if (MistexUtility.random(getLowArtefactChance(o.EP, PvPHandler.playerHasTarget(o))) == 0)
				createDrop(player, o, randomizeDrop(LOW_ARTEFACTS), LOW_ARTEFACTS);
			if (MistexUtility.random(getAncientArmourChance(o.EP, PvPHandler.playerHasTarget(o))) == 0 && getTotalNet(player) >= 76000 && getTotalNet(o) >= 76000)
				createDrop(player, o, randomizeDrop(ANCIENT_ARMOUR), ANCIENT_ARMOUR);
			if (MistexUtility.random(getMedArtefactChance(o.EP, PvPHandler.playerHasTarget(o))) == 0 && getTotalNet(player) >= 76000 && getTotalNet(o) >= 76000)
				createDrop(player, o, randomizeDrop(MED_ARTEFACTS), MED_ARTEFACTS);
			if (MistexUtility.random(getHighArtefactChance(o.EP, PvPHandler.playerHasTarget(o))) == 0 && getTotalNet(player) >= 76000 && getTotalNet(o) >= 76000)
			...
			if ((getTotalNet(player) >= 76000 && getTotalNet(o) >= 76000 && o.EP > 25 || o.playerId == player.targetIndex) && MistexUtility.random(getExtremeArtefactChance(player.EP, PvPHandler.playerHasTarget(player))) == 0)
				createDrop(player, o, randomizeDrop(EXR_ARTEFACTS), EXR_ARTEFACTS);
			/*
			 * removes the ep
			 */
			int decreaseAmount = getEarnedPotentialDecrease(o.dropWealth) + MistexUtility.random(getEarnedPotentialDecrease(o.dropWealth));
	...
	public static int getTotalNet(Client player) {
		...
		for (int j = 0; j < player.playerEquipment.length; j++) {
			totalAmount += player.getItems().getItemShopValue(player.playerEquipment[j]) * player.playerEquipmentN[j];
		}
		for (int i = 0; i < player.playerItems.length; i++) {
			if (player.playerItems[i] - 1 > 0) {
				int inventoryItemValue = player.getItems().getItemShopValue(player.playerItems[i] - 1);
