...
public class Hops {
	...
	public void doCalculations() {
		for (int i = 0; i < hopsSeeds.length; i++) {
			if (hopsStages[i] > 0 && hopsStages[i] <= 3 && Mistex.getMinutesCounter() - hopsTimer[i] >= 5) {
				hopsStages[i]--;
				hopsTimer[i] = Mistex.getMinutesCounter();
				updateHopsStates();
				continue;
			}
			...
			long difference = Mistex.getMinutesCounter() - hopsTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				hopsTimer[hopsFieldsData.getHopsIndex()] = Mistex.getMinutesCounter();
				updateHopsStates();
				if (hopsStages[hopsFieldsData.getHopsIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (hopsData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
			player.getDH().sendStatement("You need a farming level of " + hopsData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (player.getItems().getItemAmount(hopsData.getSeedId()) < hopsData.getSeedAmount()) {
			player.getDH().sendStatement("You need atleast " + hopsData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (hopsHarvest[hopsFieldsData.getHopsIndex()] == 0) {
					hopsHarvest[hopsFieldsData.getHopsIndex()] = (int) (1 + (START_HARVEST_AMOUNT + MistexUtility.random(END_HARVEST_AMOUNT - START_HARVEST_AMOUNT)) * (1));
				}
				...
				player.getItems().addItem(hopsData.getHarvestId(), 1);
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (hopsStages[hopsFieldsData.getHopsIndex()] == 0) {
			player.getDH().sendStatement("This is a hops patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (hopsStages[hopsFieldsData.getHopsIndex()] == 3) {
			player.getDH().sendStatement("This is a hops patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (hopsStages[hopsFieldsData.getHopsIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[hopsStages[hopsFieldsData.getHopsIndex()] - 4]);
