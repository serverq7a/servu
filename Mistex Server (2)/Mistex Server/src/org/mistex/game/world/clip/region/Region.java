package org.mistex.game.world.clip.region;
...
public class Region {
	...
	public static void addObject(int objectId, int x, int y, int height, int type, int direction) {
		...
		Region r = getRegion(x, y);
		if (r != null) {
			if (!objectExists(objectId, x, y, height))
				realObjects.add(new Objects(objectId, x, y, height, direction, type, 0));
		}
	}
	...
	public static void load() {
		System.out.println("Now loading regions... This may take a while!");
		try {
			...
			for (int i = 0; i < size; i++) {
                ...
                System.out.println(i + "/" + size + " map files parsed.");
                try {
